...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    let newchild = dbRef.child("Posts").childByAutoId()
    newchild.setValue(postDict)
        ...
        if snapshot.exists() {
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for (postKey, postValue) in posts {
                        let readPost = ids.contains(postKey)
                        let postToAppend = Post(id: postKey, username: postValue[firUsernameNode] as! String, postImagePath: postValue[firImagePathNode] as! String, thread: postValue[firThreadNode] as! String, dateString: postValue[firDateNode] as! String, read: readPost)
                        postArray.append(postToAppend)
                    }
